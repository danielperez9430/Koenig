...
function scopeInnerRules(css: string, _namespace: string): string {
    // Don't recurse into @media blocks — the selectors inside already
    // match the outer scoped rules. Recursing would cause infinite loops
    // because scopeCss → scopeInnerRules → scopeCss → ...
    return css;
}
